...
func kuzzle_document_search(d *C.document, index *C.char, collection *C.char, body *C.char, options *C.query_options) *C.search_result {
	...
	return goToCSearchResult(d.k, res, err)
}
...
func kuzzle_document_mget(d *C.document, index *C.char, collection *C.char, ids **C.char, l C.size_t, options *C.query_options) *C.string_result {
	res, err := (*document.Document)(d.instance).MGet(C.GoString(index), C.GoString(collection), cToGoStrings(ids, l), SetQueryOptions(options))
	s := string(res)
	return goToCStringResult(&s, err)
}
